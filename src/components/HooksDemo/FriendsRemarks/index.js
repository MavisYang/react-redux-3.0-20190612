...
export function RemarksReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case 'EDIT':
            return {
                ...state,
                list: action.data.filter(v => v.priority !== 0).map(v => ({
                    ...v,
        ...
        case 'SELECT':
            return {
                ...state,
                list: action.data.filter(v => v.priority === 0).concat(action.data.filter(v => v.priority !== 0)).map(v => ({
                    ...v,
                    color: remarksData[v.priority].color,
...
export function EditRemarks(props) {
    ...
    useEffect(() => {
        console.log(props.remarksList, 'useEffect')
        dispatch({
            type: 'EDIT',
            data: props.remarksList
        })
...
export function SetRemarks(props) {
    ...
    useEffect(() => {
        console.log(props.remarksList, 'useEffect-SetRemarks')
        // dispatch({
        //     type:'SELECT',
        //     data:props.remarksList
        // })
...
export function RemarksFilter(props) {
    ...
    useEffect(()=>{
        ...
        //     data:props.remarksList
        // })
        console.log(state,'state--RemarksFilter')
    },[state])
...
function useRemarks(remarksList) {
    ...
    useEffect(() => {
        setList(remarksList.filter(v => v.priority === 0).concat(remarksList.filter(v => v.priority !== 0)).map(v => ({
                ...v,
                color: remarksData[v.priority].color,
                name: v.name === '' ? remarksData[v.priority].name : v.name,
                select: v.select ? v.select : false
    ...
    }, [remarksList]) //添加第二个参数 可实现didUpdateMount
