...
import {fetchDataAxios} from '../StudyDemo/Fetch'
export default function React_1029() {
    ...
    return (
        <Fragment>
            <h2>React Hooks(2019.11.01 useEffect使用指南)</h2>
            <div>计时器：{count}</div>
...
function SearchResults() {
    ...
    return (
        <Fragment>
            <h4>react</h4>
            {
                isLoading&&data.slice(0, 5).map((v, i) => <a style={{display:'block'}} href={v.url} target='_blank' key={i}>{v.title}</a>)
            }
            <h4>redux</h4>
            {
                isLoading&&data1.slice(0, 5).map((v, i) => <a style={{display:'block'}} href={v.url} target='_blank' key={i}>{v.title}</a>)
            }
            {
                isError&&<div>出错啦～</div>
            }
